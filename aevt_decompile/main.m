...
//
// Because of the dearth of run-only AS samples, this code will undoubtedly need
// revision in light of further samples.
...
//      ****  Bear in mind macOS restrictions on code writing to ~/Desktop, so
//      change as preferred; ****
...
int main(int argc, const char *argv[]) {
  @autoreleasepool {
    ...
    NSString *contents =
        [NSString stringWithContentsOfFile:[path stringByExpandingTildeInPath]
                                  encoding:NSUTF8StringEncoding
                                     error:&error];
    NSString *funcParam; // Hold function paramaters;

    if (!contents) {
      NSLog(@"Failed to read contents of %@ with: %@\n", path,
    ...
    NSLog(@"Decompiling %@", path);

    for (NSString *thisLine in lines) {

      if ([thisLine containsString:CNSTNT]) {

        if ([thisLine containsString:PUSLIT] ||
            [thisLine containsString:PUSGLB]) {
          // extract the constant hex value, convert to AEVT and look up AEVT
          // code;
          ...
          if (components.count > 0) {
            ...
            if (last.length > 7) {
              ...
              NSString *hex =
                  [last substringWithRange:NSMakeRange(2, last.length - 5)];
              ...
              if (aevt.length < 1) {
                ...
                if (aevtStr.length == 8) {
                  ...
                  if (aevt.length < 1) {
                    aevt = [NSString
                        stringWithFormat:@"String Constant:  '%@'", aevtStr];
                  }
                } else {
                  aevt = [NSString
                      stringWithFormat:@"String Constant:  '%@'", aevtStr];
        ...
        NSString *intnumStr;
        NSRange endLoc = [thisLine rangeOfString:@">"
                                         options:NSBackwardsSearch];

        if (endLoc.location != NSNotFound) {
          NSRange startLoc = [thisLine rangeOfString:@"="
                                             options:NSBackwardsSearch];

          if (startLoc.location != NSNotFound) {

            intnumStr = [thisLine
                substringWithRange:NSMakeRange(startLoc.location +
                                                   startLoc.length + 2,
                                               endLoc.location - endLoc.length -
                                                   (startLoc.location + 2))];
            ...
            [output
                appendString:[NSString
                                 stringWithFormat:
                                     @"%@  ;  Decimal value = %lu\n", thisLine,
                                     strtol(intnumStr.UTF8String, NULL, 16)]];
          } else {
            // return unchanged if error;
            [output appendString:[NSString stringWithFormat:@"%@\n", thisLine]];
          }
        ...
        if ([thisLine containsString:@".app\\x"]) {
          ...
          if (preRange.location != NSNotFound) {
            ...
            NSRange appNameEnd = [thisLine rangeOfString:@".app\\x"
                                                 options:NSBackwardsSearch];

            if (appNameStart.location != NSNotFound &&
                appNameEnd.location != NSNotFound) {

              NSString *appName = [thisLine
                  substringWithRange:NSMakeRange(appNameStart.location,
                                                 (appNameEnd.location -
                                                  appNameStart.location))];
        ...
        if (prefixRange.location != NSNotFound) {
          ...
          if (startRange.location != NSNotFound) {
            ...
            NSString *stringVal = [thisLine
                substringFromIndex:startRange.location + startRange.length];
            NSRange endLoc = [stringVal rangeOfString:@">"
                                              options:NSBackwardsSearch];
            ...
            if (endLoc.location != NSNotFound) {
              ...
              if (funcParam.length < 3) {
                ...
                if (startRange2.location != NSNotFound) {
                  ...
                  NSString *stringVal2 =
                      [thisLine substringFromIndex:startRange2.location +
                                                   startRange2.length];
                  NSRange endRange2 =
                      [stringVal2 rangeOfString:@">" options:NSBackwardsSearch];
                  NSString *retVal2 =
                      [stringVal2 substringToIndex:endRange2.location];
                  [output
                      appendString:[NSString
                ...
                NSString *x =
                    [funcParam stringByReplacingOccurrencesOfString:@"\\x"
                                                         withString:@"\n"];
                ...
                for (NSString *i in xs) {
                  ...
                  NSString *ii =
                      [i stringByTrimmingCharactersInSet:
                              [NSCharacterSet
        ...
        if ([thisLine containsString:@"aevt"] &&
            [thisLine containsString:@"oapp"]) {
          NSRange oaPrefix = [thisLine rangeOfString:@": <"];

          if (oaPrefix.location != NSNotFound) {

            NSString *oaPrefixStr =
                [thisLine substringToIndex:oaPrefix.location + 2];
            [output
                appendString:[NSString
                                 stringWithFormat:@"%@ 'Open Application' \n",
                                                  oaPrefixStr]];
          ...
          if (eid.location != NSNotFound) {
            ...
            NSString *val =
                [thisLine substringFromIndex:eid.location + eid.length];
          ...
          if (decodedStr.length > 0) {
            ...
            NSString *o = [output
                stringByReplacingOccurrencesOfString:
                    [NSString stringWithFormat:@"#  ;  String: %@", funcParam]
                                          withString:
                                              [NSString
                                                  stringWithFormat:
                                                      @"  ;  String: %@\n\t  "
                                                      @"Decoded String: '%@'",
                                                      funcParam, decodedStr]];
        ...
        [output appendFormat:@"%@ ; Reference to Self (the execution parent)\n",
                             thisLine];
    ...
    for (NSString *f in finalArray) {
      ...
      if ([f containsString:findPattern1] || [f containsString:findPattern2]) {
        ...
        NSString *replacementStr =
            [f stringByReplacingOccurrencesOfString:replPattern_1_2
                                         withString:remove];

        if ([replacementStr containsString:findPattern3]) {
          replacementStr = [replacementStr
              stringByReplacingOccurrencesOfString:findPattern3
                                        withString:findPattern6];
        }

        if (![replacementStr containsString:findPattern4] &&
            ![replacementStr containsString:findPattern5]) {
          replacementStr =
              [replacementStr stringByReplacingOccurrencesOfString:findPattern6
                                                        withString:space];
        }
    ...
    Bear in mind macOS restriction on code writing to ~/Desktop, so change as
    preferred;
    */
    NSString *user = NSUserName();
    NSString *homeDir = NSHomeDirectoryForUser(user);
    NSString *outFile =
        [NSString stringWithFormat:@"%@/Desktop/%@.out", homeDir, sourceFile];
    NSURL *w = [NSURL fileURLWithPath:outFile];

    if (![finalisedStr writeToURL:w
                       atomically:true
                         encoding:NSUTF8StringEncoding
                            error:&wrErr]) {
      NSLog(@"%@", wrErr);
    }
    NSLog(@"Decompiled disasm has been written to %@", outFile);
  }
  return 0;
}
...
NSArray *ae_frameworks() {
  ...
  if (sdkDir == nil) {
    ...
    NSPredicate *filter =
        [NSPredicate predicateWithFormat:@"self ENDSWITH '.sdk'"];
...
NSString *aevt_divide(NSString *hexStr) {
  ...
  if (hexStr.length == 8) {
    ...
    NSString *concat =
        [NSString stringWithFormat:@"String Constant: '%@':\n%@\n%@", hexStr,
                                   firstReadable, secondReadable];
...
NSString *human_readable_code(NSString *searchStr) {
  ...
  if (sources.count == 0) {
    NSLog(@"aevt_decompile: Install Xcode Command Line Tools to fully "
          @"decompile all AEVT codes.\n");
  }
  ...
  for (NSString *aevt in sources) {
    ...
    NSString *fileContents =
        [NSString stringWithContentsOfURL:aevtURL
                                 encoding:NSUTF8StringEncoding
                                    error:&err];

    if (fileContents != nil) {

      NSArray *fileLines = [fileContents
          componentsSeparatedByCharactersInSet:NSCharacterSet
                                                   .newlineCharacterSet];
      ...
      for (NSString *ln in fileLines) {
        ...
        if ([ln containsString:formatTypeA] ||
            [ln containsString:formatTypeB]) {

          if (![resultStr containsString:ln]) {
            [resultStr
                appendString:[NSString stringWithFormat:@"\t%@ --> in %@\n", ln,
                                                        aevtName]];
          }
        }
      }
...
// TODO: we can improve this to actually fetch the codes rather than just
// suggest the path;
NSString *locateAppSDEF(NSString *str) {
  ...
  if (![appStr containsString:@"/System Events"] &&
      ![appStr containsString:@"/Terminal"]) {
    ...
    // TODO: A little fragile; it assumes the SDEF is named after the
    // application, which is not always true; Better to enumerate and return any
    // sdefs in the Resources folder
    NSString *sdefFile =
        [NSString stringWithFormat:@"%@/Contents/Resources/%@.sdef", app,
                                   [appStr lastPathComponent]];
